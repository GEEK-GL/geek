...
```bash
./target/release/geek --dev
```

Purge the development chain's state:

```bash
./target/release/geek purge-chain --dev
```

Start the development chain with detailed logging:

```bash
RUST_BACKTRACE=1 ./target/release/geek -ldebug --dev
```
...
(`cargo build --release && ./target/release/geek --dev --ws-external`)
by appending your own. A few useful ones are as follow.

```bash
# Run Substrate node without re-compiling
./scripts/docker_run.sh ./target/release/geek --dev --ws-external

# Purge the local dev chain
./scripts/docker_run.sh ./target/release/geek purge-chain --dev
